...
/**
 * Turns a message body into HTML that is safe to hand to a JavaScript-disabled {@code WebView}.
 * WebView has no network interceptor, so <strong>this class is the security boundary</strong>: any
 * content it lets through will be rendered verbatim.
 *
 * <p>Design invariant: the jsoup {@link Cleaner} is the enforcer. The pre-pass only <em>rewrites</em>
 * inline ({@code cid:}) images to {@code data:} URIs and <em>counts</em> remote images; it never
 * relies on its own string checks to remove anything unsafe. Whatever the pre-pass misses must still
 * die in the Cleaner. JavaScript is never enabled, in either mode.
 */
public final class Sanitizer {
	...
	/**
	 * Sanitized body HTML plus how many remote images were blocked (drives the "N remote images
	 * blocked" bar). Zero in {@link Mode#ALLOW_REMOTE}, where nothing is blocked.
	 */
	public record SafeHtml(String html, int blockedRemoteCount) {
	}

	/**
	 * Sanitizes a parsed message's body. A text-only message (no HTML body) is rendered as its
	 * HTML-escaped plain text wrapped in {@code <pre>}.
	 *
	...
	/**
	 * The core pipeline: pre-pass (rewrite {@code cid:} images, count remote images) then the jsoup
	 * {@link Cleaner}. Package-private and free of parser types so the hostile-input suite can drive
	 * it with plain strings and a synthetic {@code cid ->} data-URI map.
	 *
	...
	/**
	 * Wraps HTML-escaped plain text in {@code <pre>}. Escaping is mandatory: plain text can contain
	 * literal {@code <script>} that must never become live markup.
	 */
	...
	/**
	 * In default mode, delete any {@code style} attribute that can fetch a remote resource or hide one
	 * behind a CSS escape ({@code url}, {@code expression}, {@code image-set}, {@code @}, or a
	 * backslash). Plain colour/font/layout styles (including {@code rgb(...)}) survive.
	 */
	private static void stripFetchingStyles(Document doc) {
		for (Element el : doc.select("[style]")) {
			...
			if (value.contains("url") || value.contains("expression") || value.contains("image-set")
