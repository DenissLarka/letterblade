...
/**
 * Immutable view-model of a parsed {@code .msg} message - everything the UI needs, and nothing that ties it to the
 * parser's mutable {@code OutlookMessage}. Produced by {@link MsgService#parse}.
 *
 * <p>The collection components are defensively copied to unmodifiable collections in the canonical constructor. Their
 * <em>elements</em> are the parser's attachment objects, shared by reference on purpose: they carry the message's
 * {@code byte[]} payloads, so the model holds them rather than duplicating the bytes.
 *
