...
/**
 * Parses an Outlook {@code .msg} file into an immutable {@link ParsedMessage}. Synchronous and free of any JavaFX
 * dependency - running it off the FX thread is the caller's job (the UI does so via the toolkit executor).
 */
public final class MsgService {
     ...
     * @throws MsgParseException if the file is not a readable Outlook message (corrupt, wrong format, unreadable) - the
     *     single checked exception the UI catches
     */
    ...
    private static ParsedMessage map(OutlookMessage msg) {
        ...
        return new ParsedMessage(
                ...
                body.plainText(),
                body.branch(),
                msg.fetchTrueAttachments(),
                embedded,
                msg.fetchCIDMap());
    }

    /**
     * The message's sent time as an {@link Instant}: client-submit time if present, else the message date. The parser
     * exposes these only as {@code java.util.Date}, so the conversion happens here at the boundary and nothing but
     * {@code java.time} escapes into the model.
     *
    ...
    /**
     * Chooses the body to render and its plain-text projection, mirroring how Outlook stores bodies: a native HTML body
     * wins, otherwise the RTF-to-HTML conversion, otherwise plain text only. Pure and side-effect free so every branch
     * is unit-testable without a real message.
     *
