...
/**
 * Letterblade's main window content and its controller: toolbar, envelope header, blocked-content
 * bar, body surface (WebView / TextArea), and the bottom status strip. Opening a file parses it off
 * the FX thread via {@link FxExec}, then renders the result; opening another file replaces the
 * current window's content.
 *
 * <p>Not an {@code Application} subclass, so it can be built and driven headlessly. The security
 * boundary is respected here: the WebView has JavaScript disabled and only ever receives content via
 * {@code loadContent} of {@link Sanitizer}-cleaned HTML.
 */
public final class MainView {
	...
	public MainView(FxExec exec, MsgService msgService, Sanitizer sanitizer, Notifications notifications,
			StatusBarModel statusBarModel, Consumer<String> titleUpdater) {
	...
	/**
	 * Parses {@code file} off the FX thread and shows it; a failure raises a friendly error toast and
	 * leaves the app running. Call on the FX thread.
	 */
	public void openFile(File file) {
		FxThreads.requireFx();
		Objects.requireNonNull(file, "file");
		exec.supply(file.getName(), () -> msgService.parse(file))
				.whenCompleteAsync((parsed, error) -> {
					if (error == null) {
						showMessage(parsed);
					} else {
						notifications.error("Could not open " + file.getName()
								+ " - not a readable Outlook .msg file");
					}
	...
	private void chooseAndOpen() {
		...
		final File file = chooser.showOpenDialog(root.getScene() == null ? null : root.getScene().getWindow());
		if (file != null) {
			openFile(file);
		}
	}
	...
	private void showMessage(ParsedMessage message) {
		...
		titleUpdater.accept(isBlank(message.subject())
				? "Letterblade" : message.subject().strip() + " - Letterblade");
	...
	private void updateHeader(ParsedMessage message) {
		subjectLabel.setText(isBlank(message.subject()) ? "(no subject)" : message.subject().strip());
